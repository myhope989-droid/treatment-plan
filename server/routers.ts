  ...
  plan: router({
    ...
    uploadAndAnalyze: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        let analysisPrompt = `أنت محلل كشوف طلابية متخصص. انظر إلى هذا الكشف الدراسي واستخرج منه:
1. أسماء الطلاب الذين أمام أسمائهم رمز دائرة (○) أو دائرة مع خط (Ø) أو أي علامة تشير إلى غياب أو عدم تسليم.
2. حدد لكل طالب:
   - هل أمامه رمز الاختبار (دائرة فقط ○ = لم يحل الاختبار)
   - هل أمامه رمز المشروع (دائرة مع خط Ø = لم يسلم المشروع)
   - هل أمامه الرمزان معاً
...
مهم جداً:
- استخرج فقط الطلاب الذين يحتاجون خطة علاجية (أمامهم رمز)
- اكتب الأسماء كاملة كما هي في الكشف
